...
This is the most prevalent I/O model supported by all of I/O devices. It is also the default behavior to open a file or access a file(`read` or `write` system calls) in libc standard library. If we open a file via blocking I/O(e.g: set `O_SYNC` flag), no `read` or `write` will complete until the data is transferred to user-space application buffer from kernel buffer or already delivered to physical deivce. The process is to block until the requested action is completed. Apparently, as a result of this kind of operation manner, the I/O operations are bound to a specifc thread/process. Imaging that if we want to write a program to handle multiple connections simultaneously via blocking I/O calls, we will fall into `thread-per-connection` programming model, whichwe can find in most of early web server, like Apache.
