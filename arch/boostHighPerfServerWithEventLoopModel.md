...
Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound to a specifc thread/process during the I/O execution. This is fine as long as the I/O connections are short-lived and data link latencies are not bad. But imaging that if we want to write a program in blocking I/O way to handle multiple long-lived connections simultaneously, especially, consider the situation with bad data link latencies, we will eventually fall into `thread-per-connection` programming model to avoid the serialized I/O operations for the concurrency connections. `thread-per-connection` can be found in most of very early web server implementation, like Apache.
```
TODELETE
