...
As you may notice in this sample, the difficulty with nonblocking file descriptor calls is that there is no way of knowing when one would succeed, except by continuiosly and periodically iterating all of interested file descriptors and check available input(or writable buffer for output) simply by attemping `read` or `write` system call (a process known as “polling”). This result in the I/O operation requiring that the application make numerous `read` and `write` system calls to await completion. Obviously, this is extremely inefficient because it puts application in busy-wait(needlessly consume CPU cycles by calling `read` and `write`) until the data is available or attempt to do other work while the command is performed in the kernel. In addition, this method can introduce latency in the I\/O because any gap between the data becoming available in the kernel and the user calling `read` to return it can reduce the overall data throughput.
