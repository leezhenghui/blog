...
Inspired by the _**data selector**_ idea from hardware side, the `I/O multpliexing` is worked out to increase the amount of I/O operations, means `read` or `write` system calls on multiple files(corresponding to the several analog or digital inputs in electronincs case) by a single thread/process(corresponding to the _single line_ in electroinics case) via a selector mechanism. The selector can track readiness state change for certain I/O operation(`read` or `write`) in an efficient way provided by underlying operating system.
