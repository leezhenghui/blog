...
The diagram above illuminate the procedure, when the process make a read() or write() system call,  the process will be locked, a context switch from usser space to kernel space occurs underhood indeed, after either the data copied from device to kernel buffer(in the case of `read`) or delivered to device from kernel buffer(in the case of `write`), process context will switch back and unlock process in user space and fetch the operation result from kernel buffer to user-space application buffer. We say the process is blocked entire time from when it calls until it turns.

Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound/blocked to a specifc thread/process during the I/O state staying in read or write. In order to limit(avoid) the effect of serialized I/O operations for the different concurrent connections in blocking I/O model,  we will eventually fall into `thread-per-connection` programming model. This is fine as long as the I/O connections are short-lived and data link latencies are not bad. But if we want to write a program to handle multiple long-lived and high latency connections simultaneously, we can image that a large number of threads/processes are held/blocked by the long-lived and high latency connections in state of read/write, we have to spwan new threads/processes to service the new connections, in the worst situation, we might fall into the situation of servicing each connection by a new thread/process, then program become pretty resource intensive and CPU context switch are also highly loaded. 
