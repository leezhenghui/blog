...
Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound to a specifc thread/process. Imaging that if we want to write a program to handle multiple connections simultaneously via blocking I/O calls, we will fall into `thread-per-connection` programming model, which we can find in most of early web server, like Apache.
```
TODELETE
