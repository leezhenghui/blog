...
Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound to a specifc thread/process during the I/O execution. Imaging that if we want to write a program to handle multiple connections simultaneously via blocking I/O calls, we will eventually fall into `thread-per-connection` programming model to avoid the serialized I/O operations for the concurrency connections. `thread-per-connection` can be found in most of very early web server implementation, like Apache.
```
TODELETE
