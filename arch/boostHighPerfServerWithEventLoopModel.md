...
Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound/blocked to a specifc thread/process during the I/O staying in read or write state. In order to limit(avoid) the effect of serialized I/O processing for the different concurrent connections,  we will eventually fall into `thread-per-connection` programming model. This is fine as long as the I/O connections are short-lived and data link latencies are not bad. But if we want to write a program to handle multiple long-lived and high latency connections simultaneously in blocking I/O mode, we can image a large number of threads/processes are held up and blocked by the long-lived and high latency connections which are in state of read/write, if a fixed size thread/process pool is used, we will see it become drain, otherwise, we have to spwan new threads/processes to service the new connections, in the worst situation, we might fall into the situation of servicing each connection by a new thread/process, with the concurrency increasing, the program become more and more resource intensive and CPU context switch are also highly loaded. 
