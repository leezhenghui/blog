...
Apparently, as a result of the operation manner in blocking I/O, the system calls relevent to I/O operations are bound/blocked to a specifc thread/process during the I/O state staying in read or write. In order to limit(avoid) the effect of serialized I/O operations for the different concurrent connections,  we will eventually fall into `thread-per-connection` programming model. This is fine as long as the I/O connections are short-lived and data link latencies are not bad. But if we want to write a program to handle multiple long-lived and high latency connections simultaneously in blocking I/O way, we can image that a large number of threads/processes are held/blocked by the long-lived and high latency connections in state of read/write, if we use a fixed size thread pool, we will see it become drain shortly, or we have to spwan new threads/processes to service the new connections, in the worst situation, we might fall into the situation of servicing each connection by a new thread/process, so that the program become pretty resource intensive and CPU context switch are also highly loaded. 
