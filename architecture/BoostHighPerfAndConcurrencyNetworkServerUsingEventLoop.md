...
Finally get to the problem -- C10K. 15 Years ago, xxx arise C10K problem which was a big chellenge( This situation is often called the c10k problem. With select() or poll(), your network server will hardly perform any useful things but wasting precious CPU cycles under such high load. C10K was raised at time based on that time condition/situation(both hardware and interenet environment)..Today, C10K problem itself is not a problem anymore, people are resolving the challenge of C10M, but the insights/solution for C10K as the foundation of so many perfect softwares still enlighten us and point us to a way forward. 
