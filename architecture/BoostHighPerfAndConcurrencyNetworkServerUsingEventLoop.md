...
We can also extend those idea/concept to a higher programming level, means, we need to check, if the I/O facility can return back response/error to consumer side without consumer to trigger a "ask for" action, that is async, otherwise, sync
