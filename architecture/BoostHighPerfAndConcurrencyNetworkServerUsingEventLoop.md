...
Before we jump into event-loop programming model, we need to step back and take a look at the bigger picture, explorering the differences among the 5 classic I/O models and each appropriate programming models.
...
~~~
We just explorer the 5 typical I/O models with examples from operating system level, which explain the asynchronous and synchronous behaviors from the perspective of user space and kernel space. Actually, the two terms can also be adopted to higher level programming languages. Just understand the asynchronous means "after make the call, the process will not in pending status, once the response arrived, the datagram will be transferred by underlying system component, and a compeletion event carrying response should be notified to process."
~~~
