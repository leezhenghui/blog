...
In the book of Unix network programming, it is talking about the I/O from operating system perspecitve. so the anaylisis need to check how the response datagram get back from kernel space to user space. From kernel support perspecitve, it depends that fact of whether the data is carried by from kernel space to user space under background.
...
## Recap C10K problem 
      With the warm-up of above concepts, we finally get to the problem. 15 Years ago, xxx arise C10K problem which was a big chellenge( This situation is often called the c10k problem. With select() or poll(), your network server will hardly perform any useful things but wasting precious CPU cycles under such high load. Today, C10K problem itself is not a problem anymore, people are resolving the challenge of C10M, but we still need to show respect to C10K, as C10K was raised based on the hardware situation at that time, in additinal the solution for C10K is still take the very important responsible in today's interent servers. 
