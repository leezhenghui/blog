...
We can also extend those idea/concept to a higher software layer,and check the I/O facility we used in that layer by the similar way, whether it return response/error to consumer side without consumer to trigger a "ask for" action, that is async, otherwise, sync
